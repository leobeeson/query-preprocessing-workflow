#!/usr/bin/env python3
"""
Runner for PII extraction evaluation cases using the decorator pattern.
"""
...
import evals.cases.pii_extraction  # noqa: F401
...
from src.models.entity_extraction_models import PIIExtractionOutput


async def run_pii_extraction_evals(
    ...
    """
    Run evaluation cases for PIIExtractionAgent.
    ...
    registry = EvalRegistry.for_agent(
        ...
        output_type=PIIExtractionOutput
    )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Run evaluation cases for PIIExtractionAgent",
...
Examples:
  ...
  python -m evals.tests.test_eval_decorated_pii_extraction --cases full_card_details exposed_name_email
...
Available tags:
  Check the registry summary when running to see all available tags
    ...
    parser.add_argument(
        "--tags",
        nargs="+",
        help="Run cases that have ALL specified tags (AND logic). Example: --tags financial card"
    )

    parser.add_argument(
        "--cases",
        nargs="+",
        help="Run specific cases by name. Example: --cases full_card_details username_password_pin"
    )
...
async def main() -> List[EvalResult]:
    ...
    if args.cases:
        ...
        results: List[EvalResult] = await run_pii_extraction_evals(
            llm_client,
        ...
        # Run all evaluations
        results: List[EvalResult] = await run_pii_extraction_evals(llm_client)
