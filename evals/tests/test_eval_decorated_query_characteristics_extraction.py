...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Run evaluation cases for QueryCharacteristicsExtractionAgent",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Run all tests
  python -m evals.tests.test_eval_decorated_query_characteristics_extraction

  # Run tests with a single tag
  python -m evals.tests.test_eval_decorated_query_characteristics_extraction --tags feasible

  # Run tests that have ALL specified tags (AND logic)
  python -m evals.tests.test_eval_decorated_query_characteristics_extraction --tags aggregation feasible

  # Run tests with specific case names
  python -m evals.tests.test_eval_decorated_query_characteristics_extraction --cases simple_aggregation_transport grouped_ranking_merchants

Available tags:
  aggregation, basic, budget, comparison, complex, cte, dev_cases, feasible,
  grouped, infeasible, join, listing, merchant, missing, multiple_missing,
  ranking, snapshot, subscription, trend, window
        """
    )
    ...
    parser.add_argument(
        "--tags",
        nargs="+",
        help="Run cases that have ALL specified tags (AND logic). Example: --tags aggregation feasible"
    )

    parser.add_argument(
        "--cases",
        nargs="+",
        help="Run specific cases by name. Example: --cases simple_aggregation_transport grouped_ranking_merchants"
    )
...
async def main() -> List[EvalResult]:
    ...
    if args.cases:
        ...
        results: List[EvalResult] = await run_query_characteristics_evals(
            llm_client,
