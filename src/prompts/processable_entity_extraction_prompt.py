...
6. Budget Entities:
* The word "budget" is an entity - it refers to a financial concept being queried
* Related terms: "spending limit", "allowance", "spending cap"
* Extract these even when they appear in questions (e.g., "My budget?" - "budget" is what's being asked about)
...
* Scan for ALL SIX processable entity types systematically:
  - Temporal: dates, times, periods, ranges ("between X and Y"), days ("Mondays")
  - Budget: "budget", "spending limit", "allowance" (extract even in questions)
  - Category: terms from taxonomy including "restaurants", "bills", "groceries"
  - Merchant: specific business names (not types of businesses)
  - Amount: monetary values with modifiers ("over", "exactly", "between")
  - Environmental: carbon footprint, emissions references
* Do NOT extract SQL operations or general transaction verbs
...
11. Nouns referring to financial concepts are entities even in questions ("budget" in "My budget?" is the thing being asked about, not just question structure)
</extraction_principles>

<examples>
<example label="budget query with category">
...
5. Domain nouns in questions ARE entities ("budget" in "My budget?")
6. Do not attempt to normalize, correct, or validate entity values
7. Skip aggregation operations and SQL transformations completely
8. When no processable entities are found, return <response><entities></entities></response>
9. Never add confidence scores or any fields beyond type and value
</guardrails>
</instructions>
"""
    return instructions
...
<immediate_task>
Extract all processable entities from the query. Remember:
- Preserve complete phrases: "between 6am and 9am" not just "6am and 9am"
- Extract domain nouns from questions: "budget" in "My budget?" is an entity
- "restaurants" is a category (type of place), not a merchant
- Include temporal references like "Mondays", "weekends"

Check for all six entity types: temporal, budget, category, merchant, amount, environmental.

Return XML with nested entity tags containing type and value elements for each entity found.
</immediate_task>
